...
- (instancetype)initWithCurrentDate:(NSDate *)date {
    if (self = [super init]) {
        ...
        kRegisterNotify(@"pickView点击", @selector(pickCLck:))
...
- (void)pickCLck:(NSNotification *)notificaiton{
    NSDate *date = notificaiton.object;
    self.datePicker.date = date;
    [self selectCurrentDate:date];
}
...
- (NSString *)stringFromDate:(NSDate *)date {
    ...
    NSString *str = [dateFormattor stringFromDate:date];
    return str;
}
...
- (void)setupTitleBar {
    ...
    titleView.backgroundColor = [UIColor colorWithRed:218/255.0 green:76/255.0 blue:92/255.0 alpha:1];
...
- (void)setupWeekHeader {
    ...
    for (int i = 0; i < count; i++) {
        UILabel *weekdayLabel = [[UILabel alloc] initWithFrame:CGRectMake(offsetX, 5, (DeviceWidth - 10) / count, 20)];
        weekdayLabel.textAlignment = NSTextAlignmentCenter;
        weekdayLabel.text = Weekdays[i];
        
        if (i == 0 || i == count - 1) {
            weekdayLabel.textColor = [UIColor colorWithRed:218/255.0 green:76/255.0 blue:92/255.0 alpha:1];
        } else {
    ...
    UIView *lineView = [[UIView alloc] initWithFrame:CGRectMake(0, 30, DeviceWidth, 1)];
...
- (void)setupScrollView {
    ...
    [self.scrollView setFrame:CGRectMake(0, 30, DeviceWidth, self.centerCalendarItem.frame.size.height)];
...
- (void)setCurrentDate:(NSDate *)date {
    ...
    NSString *str = [self stringFromDate:self.centerCalendarItem.date];
    NSLog(@"当前日期：%@",str);
