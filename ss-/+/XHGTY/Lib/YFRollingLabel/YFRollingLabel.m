...
-(void)commonInit{
    ...
    for (int i = 0; i < _textArray.count; i++) {
        CGRect textRect = [((NSString *)_textArray[i]) boundingRectWithSize:maxSize options:NSStringDrawingUsesLineFragmentOrigin attributes:@{NSFontAttributeName:(_font ? _font : kFont(18))} context:nil];
    ...
    if(_textArray.count == 1){
        ...
        if(rect.size.width > KSelfWith){
            ...
            label.font = _font ? _font : kFont(18);
            label.tag = 100 + 2;
    ...
    if(_CanRolling){
        ...
        for (int i = 0; i < kNumberOfLabel; i++) {
            ...
            _labels[i].font = _font ? _font : kFont(18);
