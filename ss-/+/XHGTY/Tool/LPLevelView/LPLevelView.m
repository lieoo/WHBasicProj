...
- (void)drawIcons
{
    ...
    for (int i=1; i<=_maxLevel; i++) {
        //①获取图标
        UIImage *iconImg;
        if ((i <= _level) || (i-_level < 1 && !_levelInt)) {//整星
            iconImg = _iconFull;
...
- (void)touchesBegan:(NSSet *)touches withEvent:(UIEvent *)event
{
    if (_canScore) {
        ...
        if ([self.delegate respondsToSelector:@selector(changeStartLeve:)]) {
            [self.delegate changeStartLeve:_level];
        }
...
- (void)scoreAnimation
{
    ...
    //数值
    //FxLog(@"_level =%zd  (int)level = %d",_level, (int)_level);
